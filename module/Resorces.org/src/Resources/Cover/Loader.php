 ...
 */
namespace Resources\Cover;
use Zend\Log\LoggerInterface, ZendService\Amazon\Amazon;

use Resources\Code\ISBN;
...
class Loader implements \Zend\Log\LoggerAwareInterface
{
    ...
    protected function fetchFromContentType()
    {
        ...
        $iconFile =  '/usr/local/vufind/resources/public/img/' . $this->size .'/' . $this->type . '.png';
        ...
        //    array('.png', '.gif', '.jpg')
        //);
        if ($iconFile !== false && is_readable($iconFile)) {
    ...
    public function loadUnavailable()
    {
        ...
        $noCoverImage = isset($this->config->Content->noCoverAvailableImage)
            ? $this->config->Content->noCoverAvailableImage
            : null;
